...
$tab_cat = array('infoMoncoinUser','futurUser','supportUser','coordonneesUser');
if(!is_null($categorie)){
    if(in_array($categorie,$tab_cat)){
       echo $categorie();
    }else {header("Location:/utilisateur/".$categorie);}
} else informations();
?>
...
function supportUser(){
    ...
    echo '<p>Vous pouvez contacter l\'entreprise pour être mis en relation avec le vendeur : <a href="/utilisateur/footerUser?info=coordonneesUser">Coordonnées entreprise</a></p>';
    echo "<h4>Autre problème :</h4>";
    echo '<p>Pour tout autre soucis vous pouvez contacter l\'administrateur du site, ou l\'entreprise : <a href="/utilisateur/footerUser?info=coordonneesUser">Coordonnées entreprise</a></p>';
    echo "</div>";
}
function coordonneesUser(){
