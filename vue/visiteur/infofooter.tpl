...
if(!is_null($categorie)){
    if(in_array($categorie,$tab_cat)){
       echo $categorie();
    }else {header("Location:/visiteur/".$categorie);}
} else informations();
?>
...
function support(){
    ...
    echo '<p>Vous pouvez contacter l\'entreprise pour être mis en relation avec le vendeur : <a href="/visiteur/footer?info=coordonnees">Coordonnées entreprise</a></p>';
    echo "<h4>Autre problème :</h4>";
    echo '<p>Pour tout autre soucis vous pouvez contacter l\'administrateur du site, ou l\'entreprise : <a href="/visiteur/footer?info=coordonnees">Coordonnées entreprise</a></p>';
    echo "</div>";
}
